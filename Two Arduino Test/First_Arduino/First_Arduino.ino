...
// mySerial is UART interface with another arduino acting as scanner (check "Second Arduino.ino"); used to echo Rx data to scanner
...
bool MsgReader_Heartbeat(bool *pfRxMsgComplete) {
  if (*pfRxMsgComplete == false) {
    ...
    if ((Serial.readBytes(rxBuffer, 1) == 1) && (rxBuffer[0] == CHAR_STX)) {
      rxBufferSize = 1;
      while ((rxBufferSize < sizeof(rxBuffer)) && Serial.readBytes(&rxBuffer[rxBufferSize], 1) == 1) {
